...
namespace HomeWork.Controllers
{
    ...
    public class RepairShopController
    {
        ...
        public List<Television> OrderByModel() => _repair.OrderBy((t1, t2) => t1.Model.CompareTo(t2.Model));

        // упорядочивание по убыванию диагонали экрана
        public List<Television> OrderByDiagonal() => _repair.OrderBy((t1, t2) => t2.Diagonal.CompareTo(t1.Diagonal));

        // упорядочивание по мастеру, выполняющему ремонт
        public List<Television> OrderByMaster() => _repair.OrderBy((t1, t2) => t1.Master.CompareTo(t2.Master));

        // упорядочивание по владельцу телевизора
        public List<Television> OrderByOwner() => _repair.OrderBy((t1, t2) => t1.Owner.CompareTo(t2.Owner));

        // упорядочивание по возрастанию стоимости ремонта
        public List<Television> OrderByPriceAscend() => _repair.OrderBy((t1, t2) => t1.Price.CompareTo(t2.Price));

        // упорядочивание по убыванию стоимости ремонта
        public List<Television> OrderByPriceDescend() => _repair.OrderBy((t1, t2) => t2.Price.CompareTo(t1.Price));

...
{
    // Класс Контроллер обработки ремонтной мастерской по заданию 1
    internal class RepairShopController
    {
        ...
        public int Count => Televisions.Count;
        ...
        public List<TelevisionModel> OrderByModel() => Televisions.OrderBy(item => item.Model).ToList();


        // упорядочивание по убыванию диагонали экрана
        public List<TelevisionModel> OrderByDiagonalDesc() => Televisions.OrderByDescending(item => item.Diagonal).ToList();


        // упорядочивание по мастеру, выполняющему ремонт
        public List<TelevisionModel> OrderByMaster() => Televisions.OrderBy(item => item.Master).ToList();


        // упорядочивание по владельцу телевизора
        public List<TelevisionModel> OrderByOwner() => Televisions.OrderBy(item => item.Owner).ToList();

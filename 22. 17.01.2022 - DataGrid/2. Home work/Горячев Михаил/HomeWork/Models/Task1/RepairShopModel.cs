...
namespace HomeWork.Models.Task1
{
    ...
    public class RepairShopModel
    {
        ...
        // фабричный метод ремонтной мастерской
        public static RepairShopModel FactoryMethod()
        {
            // данные мастерской
            var info = Utils.GetInfoRepairShop();
        
            return new RepairShopModel { Address = info.Address, Name = info.Name };
        }
        ...
        static public List<TelevisionModel> GenerateTelevision(int n = 15)
        {
            ...
            // формирование данных
            for (int i = 0; i < n; i++)
                televisions.Add(FactoryMethodTelevisions());

            return televisions;
        }

        // фабричный метод создания телевизора
        static public TelevisionModel FactoryMethodTelevisions()
        {

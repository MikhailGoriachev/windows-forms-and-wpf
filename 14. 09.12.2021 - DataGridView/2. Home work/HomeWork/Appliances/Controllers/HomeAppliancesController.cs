...
namespace Appliances.Controllers
{
    ...
    internal class HomeAppliancesController
    {
        ...
        public void ShowTreeView(TreeView treeView, ContextMenuStrip CmsApp)
        {
            // очистка TreeView
        ...
        public void ShowDataGridView(DataGridView view, List<ElectricalAppliance> appliances)
        {
            // очистка DataGridView
            view.Rows.Clear();

            // количество элементов для вывода
            view.RowCount = appliances.Count;

            // заполнение DataGridView
            for (int i = 0; i < view.RowCount; i++)
            {
                view[0, i].Value = appliances[i].StateActive ? "Включен" : "Выключен";      // состояние
                view[1, i].Value = appliances[i].Name;              // название прибора
                view[2, i].Value = appliances[i].Power;             // мощность
                view[3, i].Value = appliances[i].Price;             // цена
                view[4, i].Value = appliances[i].Room;              // комната
            }
        ...
        public void ShowDataGridViewChangeElem(DataGridView view, List<ElectricalAppliance> listData, int index)
        {
            // прибор 
            ElectricalAppliance appliance = listData[index];

            // обновление данных элемента в отображении
            view[0, index].Value = appliance.StateActive ? "Включен" : "Выключен";       // состояние
            view[1, index].Value = appliance.Name;              // название прибора
            view[2, index].Value = appliance.Power;             // мощность
            view[3, index].Value = appliance.Price;             // цена
            view[4, index].Value = appliance.Room;              // комната
        }

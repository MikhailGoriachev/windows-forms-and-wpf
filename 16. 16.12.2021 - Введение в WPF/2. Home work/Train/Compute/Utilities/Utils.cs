...
namespace Compute.Utilities
{
    ...
    internal class Utils
    {
        ...
        // генерация пункта отправления и пункта назначения
        public static (string departure, string destination) GeneratePointsTrain()
        {
            (string departure, string destination)[] points = new[]
            {
                ...
                ("Калуга",          "Тверь"),
                ("Анапа",           "Воронеж"),
                ("Рязань",          "Ростов-на-Дону"),
                ("Новороссийск",    "Саратов"),
                ("Самара",          "Оренбург"),
            };

            return points[GetRand(0, points.Length)];
        }
    }
}
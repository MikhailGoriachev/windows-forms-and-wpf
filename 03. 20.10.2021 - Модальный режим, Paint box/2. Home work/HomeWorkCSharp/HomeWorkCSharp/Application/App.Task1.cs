    ...
    public partial class App
    {
        ...
        public void Task1()
        {
            ...
            void Point2()
            {
                ...
                // обмен местами
                SwapMinMax(pathFile, min.index, max.index);

                Console.WriteLine();

                // вывод файла
                ShowTable(pathFile, sizeof(double), ShowDoubleFile, item => item.i == min.index || item.i == max.index, nameFile);
            ...
            // обмен местами
            void SwapMinMax(string fileName, int indexMin, int indexMax)
            {
                using (FileStream fs = new FileStream(fileName, FileMode.Open, FileAccess.ReadWrite))
                {
                    ...
                    fs.Seek(indexMin * sizeof(double), SeekOrigin.Begin);

                    // чтение минимального элемента 
                    double min = br.ReadDouble();

                    // установка позиции в файле
                    fs.Seek(indexMax * sizeof(double), SeekOrigin.Begin);

                    // чтение минимального элемента 
                    double max = br.ReadDouble();

                    // установка позиции в файле
                    fs.Seek(indexMin * sizeof(double), SeekOrigin.Begin);

                    // запись максимальгного элемента 
                    bw.Write(max);

                    // установка позиции в файле
                    fs.Seek(indexMax * sizeof(double), SeekOrigin.Begin);

                    // запись минимальгного элемента 
                    bw.Write(min);
                }
            }
